...
combo_t key_combos[] = {
  ...
  [WF_TAB] = COMBO(wf_cdf_combo, KC_TAB),
  /* [ER_STAB] = COMBO(er_combo, LSFT(KC_TAB)), */

#ifdef TAP_DANCE_ENABLE
  [SD_ESC] = COMBO(sd_combo, ESC_CAP),
  [RS_ESC] = COMBO(rs_cdf_combo, ESC_CAP),
#else
  [SD_ESC] = COMBO(sd_combo, KC_ESC),
  [RS_ESC] = COMBO(rs_cdf_combo, KC_ESC),
#endif
  ...
  [LU_BSP] = COMBO(lu_cdf_combo, KC_BSPC),
  [NE_ENT] = COMBO(ne_cdf_combo, KC_ENT),
  [UY_ARIN] = COMBO(uy_cdf_combo, CC_ARIN),
  [EI_ADIA] = COMBO(ei_cdf_combo, CC_ADIA),
  [IO_ODIA] = COMBO(io_cdf_combo, CC_ODIA),
};
